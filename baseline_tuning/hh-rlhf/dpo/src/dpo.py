...
class BestValLossCheckpointCallback(TrainerCallback):
    ...
    Saves ONLY when eval_loss improves, to:
      output_dir/best_lr_{lr}_bs_{batch_size}_ep_{num_epochs}
    ...
    def on_evaluate(self, args, state, control, metrics=None, **kwargs):
        ...
        if eval_loss < self.best_eval_loss:
            ...
            ckpt_name = f"best_lr_{self.lr:.1e}_bs_{self.batch_size}_ep_{self.num_epochs}"
            best_dir = os.path.join(self.output_dir, ckpt_name)
...
def main():
    ...
    # if you use grad accumulation, include it so steps/evals line up
    ...
    # dataset -> {prompt, chosen, rejected} (NOW WITH PPO-STYLE DISK CACHING)
    ...
    # choose eval_steps to get ~num_evals evaluations over training
    ...
    if accelerator.is_main_process:
        ...
        # Use Trainer's save_model (handles any internal wrapping correctly)
