...
def main():
    ...
    ppo_epochs = cfg["training"]["num_epochs"]  # unused in pure-steps loop, but kept
    ...
    if accelerator.is_main_process and wandb_enabled:
        ...
        # wandb.define_metric("val_reward", step_metric="batch_idx")

    # data first (so PPOTrainer gets dataset at construction)
    ...
    # optional: final checkpoint (regardless of validation)
    # if accelerator.is_main_process:
    ...
    #     logger.info(f"Best val_reward={best_val_reward:.4f} at step={best_val_step}")
