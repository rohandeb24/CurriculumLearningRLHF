...
from transformers import (
    ...
    AutoModel,  # <-- added (embedding model)
    TrainingArguments,
    TrainerCallback,
)
...
from sklearn.cluster import MiniBatchKMeans  # <-- added
...
# ──────────────────────────────────────────────────────────────────────────────
# DATA: STANFORD SHP -> {"prompt","chosen","rejected"}  (ONLY THIS PART IS NEW)
# ──────────────────────────────────────────────────────────────────────────────
...
# ──────────────────────────────────────────────────────────────────────────────
# CURRICULUM: CLUSTER-BASED SORTING BY INTRINSIC DIMENSION (EASY → HARD)
# ──────────────────────────────────────────────────────────────────────────────
def add_cluster_intrinsic_dim_curriculum(
    train_ds,
    *,
    accelerator: Accelerator,
    logger: logging.Logger,
    ...
    """
    1) embed train_ds['prompt'] using HF AutoModel mean-pooling
    2) cluster with MiniBatchKMeans
    3) compute TWO-NN intrinsic dim per cluster
    4) sort train_ds easy→hard (low→high intrinsic dim)
    IMPORTANT: compute on main process only, cache indices to disk, then all ranks load/apply.
    """
    logger.info(
        "Applying cluster curriculum (easy→hard): embedding prompts, clustering, sorting by intrinsic dimension..."
    )

    os.makedirs(cache_dir, exist_ok=True)

    # cache key depends on dataset size + cluster params + embedding model/maxlen
    key_str = f"N={len(train_ds)}|K={n_clusters}|emb_model={emb_model}|maxlen={emb_max_len}"
    key = hashlib.md5(key_str.encode("utf-8")).hexdigest()
    order_path = os.path.join(cache_dir, f"cluster_curriculum_order_{key}.json")
    done_path = os.path.join(cache_dir, f"cluster_curriculum_order_{key}._DONE")

    if accelerator.is_main_process:
        if os.path.exists(done_path) and os.path.exists(order_path):
            logger.info(f"Cluster curriculum cache found: {order_path} (DONE exists). Loading order from disk.")
        else:
            ...
            for i in range(0, len(prompts), emb_batch):
                batch = prompts[i : i + emb_batch]
                enc = tok_emb(
                    batch,
                    return_tensors="pt",
                    padding=True,
                ...
                emb = (summed / lengths).detach().cpu().numpy()  # [B, D]
                all_emb.append(emb)
            ...
            # rank clusters by idim (low=easy)
            order = sorted(idim, key=idim.get)
            rank = {cid: r for r, cid in enumerate(order)}
            diff = np.array([rank[c] for c in labels], dtype=np.int64)

            # sort dataset indices by difficulty
            sorted_idx = np.argsort(diff).tolist()
            ...
            logger.info(f"Cluster curriculum order saved to {order_path} and DONE written.")
    ...
    train_ds = train_ds.select(sorted_idx)
    logger.info("Cluster curriculum applied: train set sorted by cluster intrinsic dimension (easy→hard).")
    return train_ds
...
def main():
    ...
    # ──────────────────────────────────────────────────────────────────────────
    # DATASET -> {"prompt","chosen","rejected"} (SHP)  (ONLY THIS BLOCK CHANGED)
    # ──────────────────────────────────────────────────────────────────────────
    ...
    # ------------------------ CLUSTER CURRICULUM (easy→hard by intrinsic dimension)
    cluster_cfg = cfg.get("cluster", {})
    n_clusters = int(cluster_cfg.get("n_clusters", 50))
    emb_batch = int(cluster_cfg.get("emb_batch", 512))
    emb_model = str(cluster_cfg.get("emb_model", "sentence-transformers/all-mpnet-base-v2"))
    emb_max_len = int(cluster_cfg.get("emb_max_len", max_prompt_length))

    curriculum_cache_dir = cluster_cfg.get("cache_dir", None)
    if curriculum_cache_dir is None:
        curriculum_cache_dir = dataset_dir if dataset_dir is not None else log_dir

    train_ds = add_cluster_intrinsic_dim_curriculum(
        train_ds,
        accelerator=accelerator,
        logger=logger,
        cache_dir=curriculum_cache_dir,
    ...
    if bool(cfg["dataset"].get("filter_merge_op_violations", True)):
        ...
        def _build_tokenized_answer_local(prompt: str, answer: str) -> dict:
            # Mirrors TRL DPOTrainer.build_tokenized_answer for decoder-only.
            ...
            # If tokenized prompt differs when tokenizing prompt+answer,
            # the last prompt token was merged. Drop that last token.
        ...
        def _passes_dpo_boundary_check(feature: dict) -> bool:
            # Mirrors TRL DPOTrainer.tokenize_row boundary check.
            try:
                ...
                # Any tokenization inconsistency -> drop this example.
                return False

        train_before = len(train_ds)
        train_ds = train_ds.filter(_passes_dpo_boundary_check, num_proc=1)
        train_removed = train_before - len(train_ds)
        logger.info(f"DPO boundary filter removed {train_removed} rows from train.")

        val_before = len(val_ds)
        val_ds = val_ds.filter(_passes_dpo_boundary_check, num_proc=1)
        val_removed = val_before - len(val_ds)
        logger.info(f"DPO boundary filter removed {val_removed} rows from val.")
    ...
    best_cb = BestValLossCheckpointCallback(
        ...
        hparams={
            ...
            "cluster/n_clusters": int(n_clusters),
            "cluster/emb_batch": int(emb_batch),
            "cluster/emb_model": str(emb_model),
            "cluster/emb_max_len": int(emb_max_len),
        },
    )
