...
def add_prompt_len_and_sort_curriculum(train_ds, tokenizer, max_prompt_length: int, logger: logging.Logger):
    ...
    Uses the DPO field 'prompt' (SHP conversion outputs it).
    """
...
def main():
    ...
    logger.info("Starting DPO training (SHP + length curriculum + best-by-eval-loss checkpointing)")
    ...
    # SHP-specific history length filter (kept exactly as in your SHP code)
    ...
    # ------------------------- DATA (SHP) — unchanged except adding the curriculum after split
    ...
    # best dir name: ONLY lr/bs/ep (as you requested earlier)
    ...
    # NO FINAL SAVING (best checkpoint only)
